...
function HeaderNav(props) {
  ...
  const listItems = navs.map((link, index) => {
    return (
      <a key={index} href={link.href}>
        <img src={link.imgLink} alt={link.alt} />{" "}
        {typeof link.num !== "undefined" ? (
          <div className="count">{link.num}</div>
        ) : (
          false
        )}
      </a>
  ...
  return <ul className="lists-nav">{listItems}</ul>;
}
const HeaderNavs = function () {
  return <HeaderNav navs={navs} />;
};
